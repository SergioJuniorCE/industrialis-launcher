    ...
    <main className="min-h-[100dvh] lg:grid lg:grid-cols-[220px_1fr]">
      <aside className="border-b border-line bg-panel/95 px-5 py-4 lg:min-h-[100dvh] lg:border-b-0 lg:border-r lg:py-6">
        <div className="flex items-center gap-3">
          <div className="grid size-8 place-items-center bg-signal text-ink"><Box className="size-4" /></div>
          <div>
      ...
      <section className="min-w-0 px-4 py-6 sm:px-7 lg:px-10 lg:py-9">
        ...
        <div className="mt-7">
          ...
          {isLoading && servers.length === 0 ? (
            <div className="grid h-40 place-items-center border border-line"><LoaderCircle className="size-5 animate-spin text-signal" /></div>
          ) : servers.length === 0 ? (
            <button type="button" onClick={() => setCreateOpen(true)} className="group flex w-full items-center justify-between border border-dashed border-line px-5 py-8 text-left transition hover:border-dim">
              <span>
                  ...
                  <article key={server.id} className="grid gap-4 border-b border-line py-4 sm:grid-cols-[minmax(180px,1.5fr)_1fr_1fr_auto] sm:items-center">
                    ...
                    <div className="flex items-center gap-1">
                      {server.status === "running" ? (
                        <IconButton label="Stop" disabled={busy} onClick={() => void run(server.id, "stop")}><Square className="size-3.5" /></IconButton>
                      ) : (
                        <IconButton label="Start" disabled={busy || server.status === "missing"} onClick={() => void run(server.id, "start")}><Play className="size-3.5" /></IconButton>
                      )}
                      <IconButton label="Restart" disabled={busy || server.status !== "running"} onClick={() => void run(server.id, "restart")}><RotateCw className="size-3.5" /></IconButton>
                      <IconButton label="Logs" disabled={busy || !server.containerId} onClick={() => void showLogs(server)}><FileTerminal className="size-3.5" /></IconButton>
                      <IconButton label="Remove" danger disabled={busy} onClick={() => window.confirm(`Remove ${server.name}? World data will remain on disk.`) && void run(server.id, "remove")}><Trash2 className="size-3.5" /></IconButton>
                    </div>
                  </article>
                );
              })}
      ...
      {createOpen && <CreateServer onClose={() => setCreateOpen(false)} onCreated={() => { setCreateOpen(false); void mutate(); }} onError={setActionError} />}
      {logs && (
        <Modal onClose={() => setLogs(null)} label={`${logs.server.name} logs`}>
          <section className="mx-auto flex max-h-[82dvh] w-full max-w-4xl flex-col border border-line bg-ink shadow-2xl">
            <header className="flex items-center gap-3 border-b border-line px-4 py-3">
              ...
              <div className="min-w-0 flex-1"><p className="truncate text-xs font-medium">{logs.server.name}</p><p className="font-mono text-[9px] text-dim">Last 300 lines</p></div>
              <button type="button" onClick={() => setLogs(null)} aria-label="Close logs"><X className="size-4" /></button>
            </header>
...
function CreateServer({ onClose, onCreated, onError }: { onClose: () => void; onCreated: () => void; onError: (error: string) => void }) {
  ...
  return (
    <Modal onClose={onClose} labelledBy="create-server-title">
      <form onSubmit={(event) => void submit(event)} className="mx-auto w-full max-w-lg border border-line bg-panel p-5 shadow-2xl sm:p-6">
        <div className="flex items-start gap-4">
          <div className="grid size-9 place-items-center bg-signal text-ink"><Activity className="size-4" /></div>
          <div className="flex-1"><h2 id="create-server-title" className="text-lg font-semibold tracking-tight">Provision server</h2><p className="mt-1 text-xs text-dim">Pull a GTNH image and create an isolated world.</p></div>
          <button type="button" onClick={onClose} aria-label="Close"><X className="size-4" /></button>
        </div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <label className="sm:col-span-2"><span className="mb-1.5 block text-[11px] font-medium">Server name</span><input name="name" required maxLength={64} placeholder="Assembly Line" className="h-10 w-full border border-line bg-ink px-3 text-sm outline-none transition placeholder:text-dim/50 focus:border-signal" /></label>
          <label><span className="mb-1.5 block text-[11px] font-medium">GTNH version</span><input name="version" required defaultValue="stable-latest" className="h-10 w-full border border-line bg-ink px-3 font-mono text-xs outline-none focus:border-signal" /></label>
          <label><span className="mb-1.5 block text-[11px] font-medium">Game port</span><input name="port" type="number" min="1024" max="65535" required defaultValue="25565" className="h-10 w-full border border-line bg-ink px-3 font-mono text-xs outline-none focus:border-signal" /></label>
          <label className="sm:col-span-2"><span className="mb-1.5 block text-[11px] font-medium">Memory limit</span><select name="memoryMb" defaultValue="6144" className="h-10 w-full border border-line bg-ink px-3 text-xs outline-none focus:border-signal"><option value="4096">4 GB</option><option value="6144">6 GB</option><option value="8192">8 GB</option><option value="12288">12 GB</option><option value="16384">16 GB</option></select></label>
        </div>
        <div className="mt-6 flex justify-end gap-2 border-t border-line pt-4">
          <button type="button" onClick={onClose} className="h-9 border border-line px-4 text-xs text-dim hover:text-copy">Cancel</button>
          <button type="submit" disabled={submitting} className="inline-flex h-9 items-center gap-2 bg-signal px-4 text-xs font-semibold text-ink disabled:opacity-50">{submitting && <LoaderCircle className="size-3.5 animate-spin" />} Create server</button>
        </div>
      </form>
    </Modal>
  );
